...
theorem Eqv.rtimes_inj_l_distl_inv_pure {A B C : Ty α} {Γ : Ctx α ε}
...
theorem Eqv.rtimes_inj_r_distl_inv_pure {A B C : Ty α} {Γ : Ctx α ε}
...
theorem Eqv.rtimes_inj_l_distl_inv {A B C : Ty α} {Γ : Ctx α ε}
  : A ⋊' inj_l ;;' distl_inv (φ := φ) (A := A) (B := B) (C := C) (Γ := Γ) (e := e) = inj_l
  := congrArg (wk_eff (he := bot_le)) rtimes_inj_l_distl_inv_pure

theorem Eqv.rtimes_inj_r_distl_inv {A B C : Ty α} {Γ : Ctx α ε}
  : A ⋊' inj_r ;;' distl_inv (φ := φ) (A := A) (B := B) (C := C) (Γ := Γ) (e := e) = inj_r
  := congrArg (wk_eff (he := bot_le)) rtimes_inj_r_distl_inv_pure
