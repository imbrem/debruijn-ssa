...
theorem Eqv.rtimes_inj_l_distl_inv {A B C : Ty α} {Γ : Ctx α ε} {L : LCtx α}
  : A ⋊ inj_l ;; distl_inv (φ := φ) (A := A) (B := B) (C := C) (Γ := Γ) (L := L) = inj_l := by
  rw [inj_l_eq_ret, rtimes_eq_ret, distl_inv_eq_ret, <-ret_of_seq, Term.Eqv.rtimes_inj_l_distl_inv]
  rfl

theorem Eqv.rtimes_inj_r_distl_inv {A B C : Ty α} {Γ : Ctx α ε} {L : LCtx α}
  : A ⋊ inj_r ;; distl_inv (φ := φ) (A := A) (B := B) (C := C) (Γ := Γ) (L := L) = inj_r := by
  rw [inj_r_eq_ret, rtimes_eq_ret, distl_inv_eq_ret, <-ret_of_seq, Term.Eqv.rtimes_inj_r_distl_inv]
  rfl
...
theorem Eqv.rtimes_sum_seq_distl_inv {A B C : Ty α} {Γ : Ctx α ε} {L : LCtx α}
  ...
  congr 1
  · rw [rtimes_rtimes, inj_l_coprod, <-rtimes_rtimes, seq_assoc, rtimes_inj_l_distl_inv]
  · rw [rtimes_rtimes, inj_r_coprod, <-rtimes_rtimes, seq_assoc, rtimes_inj_r_distl_inv]
